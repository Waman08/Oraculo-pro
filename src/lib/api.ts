...
// API â Multi-Exchange: Binance (primario) + CoinGecko (fallback)
...
// ---- Mapeo de sÃ­mbolos a pares de Binance ----
...
// ---- Mapeo de sÃ­mbolos a CoinGecko IDs ----
...
// COINGECKO â Funciones de precio (fallback)
...
// FUNCIONES PÃBLICAS â Intentan Binance primero, CoinGecko como fallback
 ...
 * Strategy: Binance â CoinGecko â null
...
export async function fetchAllBinancePrices(): Promise<Map<string, LivePriceData>> {
  ...
  // 1. Batch fetch from Binance (single request for all Binance-listed tokens)
    ...
    // Binance batch failed â will try CoinGecko for everything
  ...
  if (missingSymbols.length > 0) {
    ...
    if (geckoIds.length > 0) {
      ...
      try {
        ...
        // CoinGecko batch failed â these will show mock data
 ...
 * Fetch klines (candlestick data) â Binance primary
  ...
  try {
    ...
    if (e.name === "AbortError") {
      useAppStore.getState().addToast({
        ...
        message: "El servidor de IA está despertando (Cold Start). Esto puede tomar hasta 50 segundos.",
        type: "warning",
        duration: 8000
      });
    }
...
// PYTHON BACKEND â Real Quantitative Analysis
...
export async function initializeDynamicSymbols(): Promise<void> {
  ...
  try {
    ...
    if (data && data.symbols && Array.isArray(data.symbols)) {
      ...
      console.log(`[API] Cargados ${data.count} sÃ­mbolos dinÃ¡micos desde el backend.`);

    }

  } catch {

    console.warn('[API] Backend no disponible para sÃ­mbolos dinÃ¡micos. Usando lista estÃ¡tica.');
 ...
 * Returns null if the backend is unavailable â caller should fallback to JS engine.
...
export async function fetchPythonAnalysis(
  ...
  try {
    ...
    // Obtener UUID del usuario para personalizaciÃ³n anÃ³nima
...
export async function fetchPythonBacktest(
  symbol: string,
  timeframe: string = '1D',
  mode: string = 'Balanceado',
  days: number = 90,
  ...
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 20000); // Backtests can take slightly longer

    const url = `${PYTHON_API_URL}/api/backtest?symbol=${symbol.toUpperCase()}&timeframe=${timeframe}&days=${days}&mode=${mode}&feeRate=${feeRate}`;
    console.log("[fetchPythonBacktest] Requesting:", url);
    ...
    if (!res.ok) {
      console.warn(`[API] fetchPythonBacktest failed: ${res.status}`);
      return null;
    }
    ...
    console.error("[API] fetchPythonBacktest Error:", error);
    return null;
  }
}
